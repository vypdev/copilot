...
describe('lifecycle event replay', () => {
    it.each([
        ...
        ['check_suite', { action: 'completed', check_suite: { workflow_name: 'CI Check', head_sha: 'sha-1', status: 'completed', conclusion: 'failure', pull_requests: [{ number: 8 }] } }, 'blocked'],
        ['check_suite', { action: 'completed', check_suite: { workflow_name: 'CI Check', head_sha: 'sha-1', status: 'queued', conclusion: null, pull_requests: [{ number: 8 }] } }, 'reviewing'],
        ['workflow_run', { action: 'completed', workflow_run: { name: 'CI Check', head_sha: 'sha-1', status: 'completed', conclusion: 'success', pull_requests: [{ number: 8 }] } }, 'reviewing'],
