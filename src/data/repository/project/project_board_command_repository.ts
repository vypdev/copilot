...
interface ProjectFieldResponse {
  node: {
    fields?: CursorPage<SingleSelectFieldNode>;
  } | null;
}
...
export class ProjectBoardCommandRepository implements ProjectBoardCommandPort {
  ...
  private findFieldOption = async (
    ...
    for await (const page of paginateCursor(
      async (after) => {
        ...
        if (!result.node) {
          throw new Error(
            `Project ${project.id} was not found while reading single-select fields.`,
          );
        }
        return (
          result.node.fields ?? {
      ...
      const field = page.nodes.find(
        (candidate) =>
          candidate.name === fieldName && Array.isArray(candidate.options),
      );
      if (!field) continue;

      const option = field.options?.find(
        (candidate) => candidate.name === fieldValue,
      );
      if (!option) {
  ...
  private setSingleSelectFieldValue = async (
    ...
    const contentId = await this.projectBoardContentQueryPort.getProjectItemId(
      project,
      owner,
      repo,
    ...
    const target = await this.findFieldOption(
      client,
      project,
      fieldName,
      fieldValue,
    );
    const currentItem = await this.findProjectItem(
      client,
      project,
      contentId,
      fieldName,
    ...
    if (currentFieldValue?.optionId === target.optionId) {
      logDebugInfo(
        `Field '${fieldName}' is already set to '${fieldValue}'. No update needed.`,
      );
      return false;
    }
  ...
  setTaskPriority = async (
    ...
    this.setSingleSelectFieldValue(
      project,
      owner,
      repo,
      issueOrPullRequestNumber,
