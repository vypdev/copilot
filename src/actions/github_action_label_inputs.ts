...
export function readGithubActionLabelInputs(getInput: (key: string) => string): LabelValues {
    return {
        ...
        lifecycle: {
            ...
            ready: getInput(INPUT_KEYS.STATE_READY_LABEL),
            blocked: getInput(INPUT_KEYS.STATE_BLOCKED_LABEL),
            awaitingMaintainer: getInput(INPUT_KEYS.STATE_AWAITING_MAINTAINER_LABEL),
            awaitingIssueAuthor: getInput(INPUT_KEYS.STATE_AWAITING_ISSUE_AUTHOR_LABEL),
        },
    };
}
