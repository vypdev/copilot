...
Set `desired-assignees-count: 0` to disable automatic assignment. That setting
is a successful no-op before the action resolves a target number or reads
organization membership, so non-assignment routes do not fail merely because
they have no issue or pull-request target.
...
The column update uses the exact ProjectV2 item returned by GitHub when the
issue is added. It does not wait and re-list the board, so normal Project
propagation delay cannot turn a successful link into a failed run. Replays reuse
the existing item and do not create duplicates.
