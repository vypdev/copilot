...
Bugfix issues receive semantic plan and progress cards only when those states
are useful. Routine automation does not create an “Actions” summary or a
decorative image. The legacy `images-issue-bugfix` input remains accepted only
