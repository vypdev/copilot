...
Documentation issues receive semantic plan and progress cards only when those
states are useful. Routine automation does not create an “Actions” summary or a
decorative image. The legacy `images-issue-docs` input remains accepted only
...
labels: ["documentation", "docs", "priority: low"] # [!code highlight]
body:
  - type: checkboxes
    attributes:
