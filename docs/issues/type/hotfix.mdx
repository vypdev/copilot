...
The hotfix process follows these steps after creating an issue with the `hotfix` label and then adding `in-progress`:
...
does not create an “Actions” summary or add decorative media. The dashboard
uses the effective issue locale, managed PRs use
