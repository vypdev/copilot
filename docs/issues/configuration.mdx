...
- `opencode-model`: Legacy/general AI model reference in `provider/model` format. Bugbot agent execution uses the explicit `agent-model-provider` plus `agent-model` contract instead.
