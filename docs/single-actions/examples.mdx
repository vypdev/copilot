...
Run the CLI from the **repository root** (with `.env` containing `PERSONAL_ACCESS_TOKEN` and optional agent runtime variables). Commands mirror the single actions.
...
### copilot (CLI-only, execution role)
