---
title: Examples
description: Workflow and CLI examples for each single action.
---

# Examples

This page provides **concrete examples**: workflow YAML and CLI commands for each single action.

## Workflow: check progress

Run progress check for issue `123` (checkout the branch first if you need a specific branch):

```yaml
jobs:
  run-check-progress:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v5
        with:
          persist-credentials: false
          ref: feature/123-add-login   # optional: branch to analyze
      - uses: vypdev/copilot@v3
        with:
          token: ${{ secrets.PAT }}
          single-action: check_progress_action
          single-action-issue: "123"
          agent-provider: ${{ vars.AGENT_PROVIDER || 'codex' }}
          agent-model-provider: ${{ vars.AGENT_MODEL_PROVIDER || 'openai' }}
          agent-model: ${{ vars.AGENT_MODEL || 'gpt-5.6-luna' }}
```

## Workflow: Bugbot (detect potential problems)

Run Bugbot detection for issue `456` on the current checkout:

```yaml
jobs:
  run-bugbot:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v5
        with:
          persist-credentials: false
      - uses: vypdev/copilot@v3
        with:
          token: ${{ secrets.PAT }}
          single-action: detect_potential_problems_action
          single-action-issue: "456"
          agent-provider: ${{ vars.AGENT_PROVIDER || 'codex' }}
          agent-model-provider: ${{ vars.AGENT_MODEL_PROVIDER || 'openai' }}
          agent-model: ${{ vars.AGENT_MODEL || 'gpt-5.6-luna' }}
```

See [Bugbot](/bugbot) for full documentation.

## Workflow: recommend steps

Get implementation steps for issue `789` and post them as a comment:

```yaml
- uses: vypdev/copilot@v3
  with:
    token: ${{ secrets.PAT }}
    single-action: recommend_steps_action
    single-action-issue: "789"
    agent-provider: ${{ vars.AGENT_PROVIDER || 'codex' }}
    agent-model-provider: ${{ vars.AGENT_MODEL_PROVIDER || 'openai' }}
    agent-model: ${{ vars.AGENT_MODEL || 'gpt-5.6-luna' }}
```

## Workflow: think (no issue)

Run Think with a question (e.g. from `workflow_dispatch` with an input):

```yaml
on:
  workflow_dispatch:
    inputs:
      question:
        description: 'Question for Think'
        required: true
        type: string

jobs:
  think:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v5
        with:
          persist-credentials: false
      - uses: vypdev/copilot@v3
        with:
          token: ${{ secrets.PAT }}
          single-action: think_action
          # Think can use repo context; for a question you may need to pass it via an env or a custom step)
```

For a **question** from the CLI, use the **CLI** (see below); the workflow `think_action` uses repo context without a direct “question” input in the action. See [Workflow & CLI](/single-actions/workflow-and-cli).

## Workflow-owned release publication

Use the supplied release or hotfix workflow. It creates the durable operation
from bounded version, title, and changelog inputs, then later invokes
`create_tag`, `create_release`, and (for this repository)
`publish_github_action` with the exact issue, operation ID, and version.
Those commands intentionally have no standalone or local example.

## Workflow: create tag

Create a Git tag with prefix `v` (for example `v1.2.0`). In a current
deployment operation, the supplied workflow also passes the issue and operation
ID so the tag is created at the accepted production SHA:

```yaml
- uses: vypdev/copilot@v3
  with:
    token: ${{ secrets.PAT }}
    single-action: create_tag
    single-action-version: "1.2.0"
    single-action-issue: ${{ inputs.issue-number }}
    single-action-operation-id: ${{ inputs.operation-id }}
```

**Note:** This excerpt belongs inside the supplied workflow's `publish` mode;
the continuation dispatch supplies both inputs. Do not invent or hard-code an
operation ID. A call without matching durable operation state fails closed.

## Workflow: publish or update an issue comment

Create a new failure notification:

```yaml
- uses: ./
  with:
    token: ${{ github.token }}
    single-action: publish_issue_comment
    single-action-issue: "100"
    single-action-message: |
      ## Deployment failed

      [Review the workflow run](${{ github.server_url }}/${{ github.repository }}/actions/runs/${{ github.run_id }})
```

To append to a comment created earlier, also pass its ID and mode:

```yaml
    single-action-comment-id: "123456789"
    single-action-comment-mode: append
```

Copilot's own workflows use the local `uses: ./` form so pull requests exercise
the implementation under review. Distributed workflows use `vypdev/copilot@v3`,
which resolves to the latest published compatible v3 release.

## Workflow: initial setup

Run initial setup (labels, issue types, verify access). If the repo has no version tags, Copilot creates default tag **`v1.0.0`** so release/hotfix issues get a base version and do not get stuck in an "Unknown Version" loop:

```yaml
- uses: vypdev/copilot@v3
  with:
    token: ${{ secrets.PAT }}
    single-action: initial_setup
```

Often run once per repo or after adding new label/type config. On new repos, run this before creating release/hotfix issues.

---

## CLI examples

Run repository-dependent commands from the **target repository root**. `copilot setup` and `copilot doctor` prompt for the setup PAT; other GitHub commands accept `--token` or `PERSONAL_ACCESS_TOKEN` from the environment. `copilot upgrade`, `copilot --version`, and help can run from any directory. `copilot check-progress`, `copilot detect-potential-problems`, `copilot recommend-steps`, and `copilot think` correspond to supported action capabilities; `copilot setup`, `copilot doctor`, and `copilot do` are CLI-specific workflows. See [Workflow & CLI](/single-actions/workflow-and-cli) for installation, updates, and the complete option reference.

### setup

```bash
copilot setup
```

### check-progress

```bash
copilot check-progress -i 123
# Or with branch
copilot check-progress -i 123 -b feature/123-add-login
```

### detect-potential-problems (Bugbot)

```bash
copilot detect-potential-problems -i 456
# With branch and debug
copilot detect-potential-problems -i 456 -b feature/456-fix-bug --debug
```

### recommend-steps

```bash
copilot recommend-steps -i 789
```

### think

```bash
copilot think -q "Where is authentication validated?"
```

### do (CLI-only, execution role)

```bash
copilot do -p "Explain the main function"
```


## Next steps

- **[Available actions](/single-actions/available-actions)** — Full list of actions and inputs.
- **[Workflow & CLI](/single-actions/workflow-and-cli)** — CLI command reference.
- **[Configuration](/single-actions/configuration)** — All single-action inputs.
