...
operation ID. Calls without schema-v3 state retain the release-branch behavior
only for compatibility with an older in-flight deployment.

## Workflow: deployed

Resume legacy deployment issue `100` through the compatibility entry point:
...
New release and hotfix workflows do not call `deployed_action`; they use the
four internal deployment actions described in [Available actions](/single-actions/available-actions#internal-deployment-orchestration-actions).
