...
The **`copilot`** CLI command (e.g. `giik copilot -p "..."`) uses the configured execution role to analyze or modify code. There is **no** `single-action` equivalent in the GitHub Action; it is available only from the CLI. Use it for interactive or scripted code changes with AI.
