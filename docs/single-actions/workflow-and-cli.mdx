...
The package exposes one global executable: **`copilot`**. Install it from a clone of this repository, not from the repository where you intend to run a command.
...
### Reinstall or update an existing global installation
...
command -v copilot
copilot --help
corepack pnpm list --global --depth 0 copilot
```

The CLI does not currently expose a `copilot --version` option; use the global package listing to inspect its package version. If the command is not found after installation, configure the global pnpm bin directory in your shell's `PATH` and open a new shell.
