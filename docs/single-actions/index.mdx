...
| **Repository setup** | Provision labels and issue types without opening a deployment mutation path. |
...
<Info>
**Actions that fail the job** if the last step fails include all seven
...
`failed_deployment_action`. The workflow is marked as failed so you can act.
</Info>
