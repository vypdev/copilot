...
<CardGroup cols={2}>
  ...
  <Card title="Deploy lifecycle and compatibility" icon="git-merge" href="/single-actions/deploy-label-and-merge">
    Production-first orchestration, deploy/deployed meaning, internal continuations, recovery, and the bounded legacy path.
  </Card>
...
<Info>
**Actions that fail the job** if the last step fails: `publish_github_action`, `create_release`, `deployed_action`, `create_tag`, `close_inactive_issues_action`, `publish_issue_comment`, and the four internal deployment-orchestration actions. The workflow will be marked as failed so you can act on it.
</Info>

## Next steps

- **[Available actions](/single-actions/available-actions)** — Complete list with inputs and descriptions.
- **[Deploy lifecycle and compatibility](/single-actions/deploy-label-and-merge)** — Production-first lifecycle, internal continuations, labels, recovery, and legacy compatibility.
