...
2. **Branch merges** (if a release or hotfix branch is configured):
   - **Release:** starts the release-to-default (e.g. `main`) and release-to-development (e.g. `develop`) PR flows concurrently, then waits for both to succeed.
...
- Checks are polled every 20 seconds to limit GitHub API usage. A configurable **merge timeout** (input `merge-timeout`, default 600 seconds) limits how long we wait for checks **per merge**. A value of `0` means there is no artificial timeout. If a positive timeout is reached, the merge fails and no direct-merge bypass is attempted.
