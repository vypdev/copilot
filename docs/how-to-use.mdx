...
If the repo does not include the compiled `build/` folder (e.g. it is gitignored), run `corepack pnpm install` and `corepack pnpm run build` before `corepack pnpm install . --global`.
...
When a new Copilot version is available, update the checkout and reinstall the local package globally:
...
Confirm the executable and installed package:
...
The CLI currently has no `copilot --version` option. The complete command reference, including every supported option, is in [Workflow & CLI](/single-actions/workflow-and-cli).
