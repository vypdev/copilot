...
2. It asks GitHub for only the active runs of that workflow (`in_progress`, `queued`, `requested`, `waiting`, and `pending`) instead of scanning the repository's complete run history.
3. It filters runs with the **same workflow name** and a **lower run ID** (i.e. started earlier). Transient GitHub API failures are retried with exponential backoff.
4. If any such run exists, the action waits 2 seconds and checks again, up to a long timeout (~4000 seconds).
5. When no previous run of the same workflow is active, the action continues.
