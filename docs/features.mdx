...
| **Bugbot review** | Reviews the full PR on open/reopen and the new commit range on synchronize; publishes stable line findings, reconciles resolved/reopened status, and exposes a Check Run. |
...
| **`create_tag`** | `single-action-version`; orchestration also supplies issue and operation ID | In a schema-v3 deployment, creates or verifies the immutable `v<version>` tag at the accepted production SHA. The release-branch behavior exists only for legacy calls without orchestration state. |
...
| **`deployed_action`** | `single-action-issue` | Compatibility entry point for an existing deployment. With schema-v3 state it delegates to orchestration; without state it retains the bounded legacy completion path. New workflows use the internal actions below. |
...
Single actions that **throw an error** if the last step fails: `publish_github_action`, `create_release`, `deployed_action`, `create_tag`, `close_inactive_issues_action`, `publish_issue_comment`, `prepare_deployment_action`, `continue_deployment_action`, `published_deployment_action`, and `failed_deployment_action`. The four deployment-orchestration actions are internal contracts and must not be invoked manually.
