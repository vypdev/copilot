...
| **Branch synchronization** | All-branch push observer; issue/PR command | Detection never loads Bugbot or a code-changing agent. Bundled English/Spanish notice copy is local; another configured BCP-47 locale may use one schema-constrained language-catalog request. An authorized command merges parent into working branch, invokes the fixer only for eligible Git conflicts, validates the prepared merge, runs configured checks, revalidates remote heads, pushes, and reports the outcome. |
