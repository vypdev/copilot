...
| **Comment translation** | Addressed issue comment; addressed PR review comment | Translates explicitly addressed comments to the configured locale (`issues-locale`, `pull-requests-locale`) when they are written in another language. Unaddressed human and automated comments are inert. |
