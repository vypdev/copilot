...
### Performance
- Fast execution
- **Sequential workflow handling** — The action waits for earlier active Copilot/Task mutation runs, so related workflows run one after another without discarding intermediate triggers (see [Features → Workflow concurrency](/features#workflow-concurrency-and-sequential-execution)).
