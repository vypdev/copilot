...
9. **External lifecycle evidence:** `pull_request_review` and the configured `CI Check` workflow completion update only the managed lifecycle label. A failed check blocks the PR, an approved review plus a successful check marks it ready, and a requested change marks it as changes requested. These events do not invoke an agent pipeline.
