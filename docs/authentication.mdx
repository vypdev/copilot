...
default) or manual entry. The setup link contains only bootstrap permissions
known before the questionnaire; it is **provisional**. If the final plan needs
...
an individual repository. Check the active browser account, select only the
