...
endpoints are permission-bound and may verify directly. Public organization
member and issue-type reads remain `Unverifiable`.
...
cannot obtain an authoritative remote snapshot, selected Secret and Variable
upserts stop with a bounded inventory-recovery message, including when the
configured target is repository scope.
