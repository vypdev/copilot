...
denial and there is no rate-limit, retry, or SSO signal. Bare, rate-limited,
SSO-constrained, and otherwise ambiguous `403` responses remain `Unverifiable`;
raw provider messages are never printed.
...
resource at repository scope, or must discover its repository scope to preserve
an existing value, the final setup-PAT table promotes it to required and stops
...
If organization storage itself is unavailable, setup still renders and runs the
final setup-PAT permission audit before reporting the storage validation error;
plan confirmation and mutation do not begin.
