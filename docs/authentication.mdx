...
<Steps>
  ...
  <Step title="Create the workflow PAT">
    ...
    - **Checks**: Read and write (Bugbot Check Runs)
    - **Contents**: Read and write
    - **Issues**: Read and write
    - **Metadata**: Read-only
    - **Pull requests**: Read and write

    Do not grant Administration, Secrets, Variables, Webhooks, or Workflows permissions to the runtime PAT unless an independently reviewed extension actually uses them. Workflow installation and Secret/Variable administration belong to the separate setup PAT.
