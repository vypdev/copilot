...
Once installed state is proven on the selected ref, setup dispatches the known
workflow path and that ref without a second lookup in the default-branch
Actions index. Missing state still follows the bounded temporary-bootstrap
flow; unavailable state never dispatches or mutates.
