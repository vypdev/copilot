...
- **CLI:** From the repository root, run `copilot detect-potential-problems -i <issue>` (optionally `-b <branch>`). Requires a `.env` with `PERSONAL_ACCESS_TOKEN` and the configured agent CLI credentials/model. See [Examples → CLI](/bugbot/examples#cli) and [Smoke tests](/operations/smoke-tests).
