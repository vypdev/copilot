...
From an issue or pull-request comment, `/copilot review`, `/copilot findings`,
or `/copilot recheck` starts the read-only detection flow immediately. On an
issue comment, Copilot resolves the branch from the open PR linked to the
issue. These commands do not invoke autofix and do not modify files.
