...
<Tabs sync={false}>
  <Tab title="Authentication & AI" icon="key">
    ...
    - `agent-provider`: CLI runtime (`opencode`, `codex`, or `cursor`; default: `opencode`)
    ...
    - `agent-command`: optional provider-specific CLI command; OpenCode commands must include `--model provider/model`
    ...
    - `issues-locale`: Target locale for issue comments (default: "en-US"). When comments are in another language, OpenCode translates them to this locale. See [OpenCode → Comment translation](/agents/opencode#how-comment-translation-works).
    ...
    - `bugbot-fix-verify-commands`: Comma-separated commands to run after Bugbot autofix (e.g. `pnpm run build, pnpm test, pnpm run lint`). When a user asks to fix findings from an issue or PR comment, the configured agent applies fixes and these commands run before the action commits and pushes; if any fails, no commit is made. Default: empty (only OpenCode's run is used). See [Features → Bugbot autofix](/features#ai-features-opencode).
