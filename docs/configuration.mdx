...
unattended runs where identity and required reads are verified but safe probes
cannot prove required writes. It never bypasses missing or unverifiable read
access. There is no legacy configuration shape or compatibility alias: unknown keys and removed
