...
<Tabs sync={false}>
  <Tab title="Authentication & AI" icon="key">
    ...
    - `bugbot-fail-on-unresolved`: Fail the Action and its native Check when the latest review has open or reopened findings. Disabled by default, so findings are reported neutrally.
  ...
  <Tab title="Project & Behavior" icon="table-cells-large">
    ...
    - `merge-timeout`: Timeout in seconds for deprecated `legacy-wait` only
      (default: `600`; `0` disables its artificial timeout). It is accepted but
      ignored with a warning by `auto`, `auto-merge`, `merge-queue`, and
      `create-only`.
    ...
    - `reconciliation-pr-mode`: `auto` (default), `auto-merge`, `merge-queue`, `create-only`, or deprecated `legacy-wait`
