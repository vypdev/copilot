...
The current loader asks for all open PR numbers for a branch, fans comments and
...
The route projects a `BugbotReviewTarget` containing repository numeric ID,
owner/name, trigger kind, optional positive issue number, normalized head owner
and ref, expected full head SHA, and optional event PR number. It contains no
token; context ports are auth-bound in composition.
...
1. If an event PR number exists, call `getPullRequest(number)` once. Accept only
   an open PR whose base repository ID equals the target, head repository owner
   and ref exactly match the target, and current head SHA equals expected SHA.
...
- [ ] Canonical selection has no implicit ordering or fallback.
- [ ] Provider work and maximum concurrency meet every fixed budget.
- [ ] Required read failures abort before model/publication/resolution.
- [ ] Partial coverage cannot declare clean or resolve omitted findings.
- [ ] The same identity is used end to end and freshness is rechecked.
- [ ] At least 18 distinct cases and changed-module/repository thresholds pass.
- [ ] Active behavior, docs, UX, telemetry, SDD, catalog, and ports agree.
- [ ] No open decision, unbounded pagination/fan-out, legacy context/marker,
