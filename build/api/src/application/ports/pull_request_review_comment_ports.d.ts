...
export interface PullRequestReviewCommentCreatePort {
    createReviewWithComments(owner: string, repository: string, pullRequestNumber: number, commitId: string, body: string, comments: PullRequestReviewCommentDraft[], token: string): Promise<PullRequestReviewReference | undefined>;
}
...
export interface PullRequestReviewSummaryQueryPort {
    listPullRequestReviews(owner: string, repository: string, pullRequestNumber: number, token: string): Promise<PullRequestReviewSummary[]>;
}
export interface PullRequestReviewSummaryUpdatePort {
    updatePullRequestReview(owner: string, repository: string, pullRequestNumber: number, reviewIdentity: string, body: string, token: string): Promise<void>;
}
...
export interface PullRequestReviewThreadStateQueryPort {
    ...
    listPullRequestReviewThreadStates(owner: string, repository: string, pullRequestNumber: number, token: string): Promise<Record<string, PullRequestReviewThreadState>>;
}
